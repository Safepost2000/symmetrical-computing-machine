...
class GeminiLLM:
    def __init__(self, model_name: str = DEFAULT_MODEL):
        ...
        self.supports_stop_words = False  # Add missing attribute
    ...
    async def agenerate_content(self, prompt: str, **kwargs):
        """Generate content asynchronously using Gemini API."""
        try:
            response = await asyncio.to_thread(self.model.generate_content, prompt, **kwargs)
            return response.text
        except Exception as e:
            logger.error(f"Error generating content: {e}")
            return f"Error generating content: {str(e)}"
...
def create_researcher_agent() -> Agent:
    return Agent(
        ...
        llm=GeminiLLM(),
        tools=[InternetSearchTool()],  # Fixed list syntax
        verbose=True,
...
def create_analyst_agent() -> Agent:
    return Agent(
        ...
        llm=GeminiLLM(),
        tools=[DataAnalysisTool()],  # Fixed list syntax
        verbose=True,
        allow_delegation=True,
    )
...
def create_writer_agent() -> Agent:
    return Agent(
        ...
        llm=GeminiLLM(),
        tools=[ContentCreationTool()],  # Fixed list syntax
        verbose=True,
        allow_delegation=True,
    )
...
async def create_and_run_crew(user_id: int, task_type: str, task_description: str) -> str:
    try:
        ...
        crew = Crew(
            agents=agents,
            tasks=[task],  # Fixed list syntax
            verbose=True,
            process=Process.sequential,
        )
        
        user_tasks[user_id] = {  # Fixed dictionary syntax
            "crew": crew,
...
# Telegram bot handlers (truncated for brevity)
